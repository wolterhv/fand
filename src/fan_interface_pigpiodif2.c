                     ...
                     float intensity)
{
    ...
    // if (config->pwm_freq != get_PWM_frequency(state->pigpio_client_id,
    //                                           config->gpio_pin))
    // if (dc != get_PWM_dutycycle(state->pigpio_client_id, config->gpio_pin))

    set_PWM_frequency(state->pigpio_client_id,
                      config->gpio_pin,
                      config->pwm_freq);
    set_PWM_dutycycle(state->pigpio_client_id, 
                      config->gpio_pin, 
                      pwm_dc);
            ...
            struct FanIFState  *state)
{
    ...
    state->pigpio_client_id = pigpio_start((char *) config->pigpio_server_address, 
                                           (char *) config->pigpio_server_port); 
    set_PWM_frequency(state->pigpio_client_id,
                      config->gpio_pin,
                      config->pwm_freq);
...
{
    fan_if_set_intensity(config, state, FAN_INTENSITY_OFF);
    pigpio_stop(state->pigpio_client_id);
    return 0;
}
